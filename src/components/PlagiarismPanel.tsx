    ...
    <>
      <div className="p-3 border-b border-border flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button 
            ...
            size="icon" 
            onClick={onClose}
            aria-label="Close plagiarism check panel"
          >
            <X className="w-4 h-4" />
          </Button>
        </div>
      ...
      <div className="p-3 space-y-4 overflow-y-auto flex-1 scrollbar-dark">
        <Button onClick={onRun} disabled={running} className="w-full group" size="sm">
          {running ? <Loader2 className="w-4 h-4 animate-spin mr-1" aria-hidden="true" /> : <ShieldCheck className="w-4 h-4 mr-1" aria-hidden="true" />}
